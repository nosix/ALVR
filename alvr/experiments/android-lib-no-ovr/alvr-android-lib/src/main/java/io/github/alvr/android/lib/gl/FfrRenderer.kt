...
class FfrRenderer(
    ...
    width: Int,
    height: Int,
    private val ffrParam: FfrParam
) : Renderer {
    companion object {
        private val TAG = FfrRenderer::class.simpleName
    }
    ...
    private fun GlSurface.setup(width: Int, height: Int): Properties = withGlContext {
        Log.i(TAG, "setup with $ffrParam")

        GLES32.glViewport(0, 0, width, height)
        GLES32.glClearColor(1f, 0f, 1f, 1f)

        val vertexShader = loadShader(GLES32.GL_VERTEX_SHADER, VERTEX_SHADER)
        val fragmentShader = loadShader(GLES32.GL_FRAGMENT_SHADER, ffrParam.getFragmentShader())
