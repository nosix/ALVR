...
/**
 * EGLContext associated with the thread that draws external textures
 *
 * Constructor and all methods must be called in the same thread.
 */
class ExternalEGLContext(shareContext: EGLContext) : Closeable {

    companion object {
        private val TAG = ExternalEGLContext::class.simpleName
    ...
    fun createSurfaceTexture(textureId: Int, width: Int, height: Int): Texture {
        return withMakeCurrent {
            when (getTarget(textureId)) {
                GLES20.GL_TEXTURE_2D -> {
                    val internalTextureId = createGLTexture()
                    val surfaceTexture = SurfaceTexture(internalTextureId)
                    surfaceTexture.setDefaultBufferSize(width, height)
                    Texture2D(textureId, width, height, surfaceTexture, internalTextureId)
                }
    ...
    private inline fun <T> withMakeCurrent(action: ExternalEGLContext.() -> T): T {
    ...
    class Texture2D(
        ...
        override fun update() {
            ...
            val error = GLES20.glGetError()
            if (error != GLES20.GL_NO_ERROR) {
                Log.e(TAG, error.toErrorString())
            }
        }

        override fun release() {
            surfaceTexture.release()
            GLES20.glDeleteTextures(1, intArrayOf(externalOesTextureId), 0)
        }
    }
}